...
import { Handshake, Loader2, Camera, X } from "lucide-react";
...
export default function SponsorFormModal({ open, onClose, onSubmit, initial, loading }) {
  ...
  useEffect(() => {
    if (!open) return;
    const next = sponsorToForm(initial);
    setForm(next);
    setPreview(mediaUrl(next.existingLogo) || "");
    setErrors({});
  }, [open, initial]);
  ...
  return (
    <DashboardFormModal
      ...
      maxHeightClass="max-h-[min(90vh,calc(100dvh-2rem),760px)]"
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-text-muted truncate">
                {initial ? "Update sponsor details" : "Register a sponsor for your tournaments"}
              </p>
            </div>
          </div>
          <button
      ...
      footer={
        <div className="flex flex-col-reverse sm:flex-row justify-end gap-2 px-5 py-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-primary hover:bg-primary-dark text-white text-sm font-semibold disabled:opacity-60"
          >
      ...
      <div className="space-y-4 min-w-0">
        <div className="flex items-center gap-4">
          <div className="relative w-20 h-20 rounded-2xl bg-slate-100 border border-slate-200 overflow-hidden flex items-center justify-center shrink-0">
            {preview ? (
              <img src={preview} alt="" className="w-full h-full object-contain bg-white p-1" />
            ) : (
          ...
          <p className="text-sm text-text-muted">Sponsor logo (optional) · JPG/PNG/WEBP · max 5MB</p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Field label="Sponsor Name" required className="sm:col-span-2" error={errors.sponsorName}>
            <input
          ...
          <Field label="Sponsorship Amount" error={errors.sponsorshipAmount}>
            <input
              type="number"
              min={0}
              className={inputClass}
